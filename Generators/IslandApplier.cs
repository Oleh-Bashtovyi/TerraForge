...
public class IslandApplier
{
    ...
    public float RadiusAroundIslands { get; set; }
    ...
    public float[,] ApplyIslands(float[,] map)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                var dotCenter = new Vector2(x + 0.5f, y + 0.5f);
                var nearestPoint = MapHelpers.FindNearestPoint(dotCenter, islandCenters);
                var heightFactor = CalculateDistance(dotCenter.X, dotCenter.Y, nearestPoint.X, nearestPoint.Y);
